...
void setup_hardware(void) {
    ...
    gpio_config_t sensor_cfg = {
        ...
        .intr_type = GPIO_INTR_NEGEDGE // Trigger when signal drops (Object detected = neg)
    };
...
void app_main(void) {
    ...
    while (1) {
        ...
        if (object_nearby) {
    // 1. Wait a tiny bit for the 'noise' to stop
    vTaskDelay(pdMS_TO_TICKS(50)); 
    
    // 2. Re-check the physical state
    if (gpio_get_level(OBSTACLE_SENSOR_PIN) == 0) {
        printf("\n>>> [CONFIRMED] OBSTACLE DETECTED! <<<\n");
        
        // 3. Optional: Block until the hand is fully removed
        while(gpio_get_level(OBSTACLE_SENSOR_PIN) == 0) {
            vTaskDelay(pdMS_TO_TICKS(100));
        }
    }
    object_nearby = false; // Always clear the flag
}
